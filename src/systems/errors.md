...
> **خمسةً** منها فقط تحمل رموزًا: `lexical` (6) · `syntactic` (32) · `semantic` (48) ·
> `runtime` (74) · `internal` (23) = **183** رمزًا. أمّا `import` و`io` و`ownership`
> فملفّاتُ إحالةٍ بـ`errors: []` — أخطاؤها مُصنَّفةٌ تحت `runtime` (مثل `RUN008`
> للاستيراد)، أُبقيت لتوثّق الإحالة لا لتُكرِّر الرمز.
>
> ويقابلُ الـ183 في `shared/errors/include/error_codes.h` **183** عضوًا في
> `enum class ErrorCode`، كلُّ عضوٍ موثَّقٌ بتعليقٍ يحمل رمزَه المرقَّم
> (`///< SEM002: …` · `///< RUN008: …` — لا وسمَ حرفيًّا اسمُه `CODE`). فلا عضوَ بلا
> رمز، ولا رمزَ بلا عضو. النطاقاتُ أعلاه حدودٌ معماريّة لا أعدادٌ فعليّة — فئةٌ لم تبلغ
> 099 ليست ناقصة.
