...
> (وكانت تبقى نسخةٌ بالترتيب القديم في `shared/parser/src/specs/flow/parser_comprehension.cpp`
> نموذجًا ميتًا غيرَ مُترجَم — **حُذفت من الشجرة**؛ لم يعد للمسار وجودٌ في `dev`.)
...
> حجم البانِي، فلا تتقاسم بنية List/Dict المستقيمة. (ترويسةُ الملفّ ما زالت تذكر
> «SetComp and Generator» تاريخيًّا، لكنّ `buildExprGenerator` حُذف في تمّوز ٢٠٢٦
> (#205) — فالملفّ اليومَ يحمل `buildExprSetComp` وحدَها.)
